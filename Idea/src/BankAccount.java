...
while(checkSwitch) {
   System.out.print("Please enter a account number to transfer: ");
         String accountNumber = sc.next();
         System.out.print("Please enter the account name: ");
         String name = sc.next();
             for (int i = 0; i < accountNumbers.size(); i++) {
                    if (accountNumbers.get(i).equals(accountNumber) && accountNames.get(i).equalsIgnoreCase(name)) {
                           System.out.print("Please enter the how much you want to transfer: ");
                               int transferAmount = sc.nextInt();
                               if (transferAmount > balance || transferAmount < 0) {
                                    System.out.println("Insufficient balance! Your balance is " + balance + "\nYou want to transfer :" + transferAmount);
                               }
                               else {
                                    balance -= transferAmount;
                                    ...
                                    System.out.println("After transfer balance is " + accountBalances.get(i));
                                    checkSwitch = false;
                                }
                            }
                            if (!accountNumbers.get(i).equals(accountNumber) && !accountNames.get(i).equalsIgnoreCase(name)) {
                                System.out.println("You entered an invalid account number and name!");
